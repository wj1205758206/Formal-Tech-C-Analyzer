...
module P = Plugin.Register
  (struct
     let name = ""
     let shortname = ""
     let help = "General options provided by the SMAVE Code Analyzer"
   end)
...
module PrintVersion =
  False(struct
          ...
          let help = "print the SMAVE Code Analyzer"
        end)
...
module PrintShare =
  False(struct
          ...
          let help = "print the SMAVE Code Analyzer share path"
        end)
...
module PrintLib =
  False(struct
          ...
          let help = "print the path of the SMAVE Code Analyzer kernel library"
        end)
...
module PrintPluginPath =
  False
    (struct
       ...
       let help =
         "print the path where the SMAVE Code Analyzer dynamic plug-ins are searched into"
     end)
  ...
  Bool
    (struct
      ...
      let help = "when pretty-printing C code, keep prototypes coming \
                  from SMAVE Code Analyzer standard library"
  ...
  module Enable = struct
    include Bool
      (struct
         ...
         let help = "dump a journal while SMAVE Code Analyzer exit"
       end)
    ...
    (struct
      ...
      let help =
        "adds -I$FRAMAC_SHARE/libc to the options given to the cpp command. \
         ...
         inconsistent mix of system and SMAVE Code Analyzer header files"
    end)
      ...
      let help =
        if Fc_config.has_yojson then
          "when set, preprocessing of each file will include corresponding \
           ...
           NOTE: this requires SMAVE Code Analyzer to be compiled with yojson support."
        else
          "Unsupported: recompile SMAVE Code Analyzer with Yojson library to enable it"
    end)
...
let () =
  Cmdline.add_option_without_action
    ...
    ~plugin:""
    ~group:seq
    ~help:"parse options before `-then' and execute SMAVE Code Analyzer \
accordingly, then parse options after `-then' and re-execute SMAVE Code Analyzer"
    ~visible:true
    ~ext_help:""
    ()
